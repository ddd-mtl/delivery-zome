...
pub fn pull_inbox(_:()) -> ExternResult<Vec<ActionHash>> {
   ...
   for (_, _, reply) in replies_tuples {
      ...
      if reply.has_accepted && !received_parcel_ehs.contains(&reply.notice_eh) {
         ...
         unreceived_entries.insert(notice.summary.parcel_description.reference.eh.clone(), notice.clone());
         /// Get unreceived chunks
         if let ParcelKind::Manifest(_) = notice.summary.parcel_description.reference.kind_info {
            let maybe_manifest: ExternResult<ParcelManifest> = get_typed_from_eh(notice.summary.parcel_description.reference.eh);
