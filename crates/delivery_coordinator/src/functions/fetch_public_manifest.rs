...
pub fn fetch_public_manifest(manifest_eh: EntryHash) -> ExternResult<(ParcelManifest, Timestamp, AgentPubKey)> {
   let (record, manifest) = get_typed_and_record::<ParcelManifest>(AnyLinkableHash::from(manifest_eh), GetStrategy::Network)?;
   /// Emit signal
...
pub fn fetch_chunk(chunk_eh: EntryHash) -> ExternResult<ParcelChunk> {
   ...
   let (record, chunk) = get_typed_and_record::<ParcelChunk>(AnyLinkableHash::from(chunk_eh), GetStrategy::Network)?;
   /// Emit signal
   attest_entry_created(record.clone(), false)?;
   ///
   Ok(chunk)
