...
#[hdk_extern]
pub fn process_inbox(strategy: GetStrategy) -> ExternResult<Vec<ActionHash>> {
   debug!("START");
   std::panic::set_hook(Box::new(zome_panic_hook));
   /// Get all inbox items
   let pending_pairs = probe_all_inbox_items(None, strategy.clone())?;
   ...
   for (pending_item, link) in pending_pairs {
      ...
      match pending_item.kind {
         ...
         ItemKind::NoticeAck => {
            ...
            match res {
               Err(e) => warn!("{}", e),
               Ok(_) => { let _res = delete_link_relaxed(link.create_link_hash, strategy.clone().into()); },
            }
         }
   ...
   for (_, _, reply) in replies_tuples {
      ...
      if reply.has_accepted && !received_parcel_ehs.contains(&reply.notice_eh) {
         let notice: DeliveryNotice = get_typed_from_eh(reply.notice_eh, strategy.clone())?;
         ...
         if let ParcelKind::Manifest(_) = notice.summary.parcel_reference.description.kind_info {
            let maybe_manifest: ExternResult<ParcelManifest> = get_typed_from_eh(notice.summary.parcel_reference.parcel_eh, strategy.clone());
   ...
   for (eh, (entry, link)) in entry_map.iter() {
      if let Some(notice) = unreceived_entries.get(eh) {
         ...
         let maybe_el = get(link.create_link_hash.clone(), strategy.clone().into())?;
         if maybe_el.is_none() {
   ...
   for (eh, (entry, link)) in chunk_map.iter() {
      if unreceived_chunks.contains(eh) {
         ...
         let _link_hh = delete_link_relaxed(link.create_link_hash.clone(), strategy.clone().into())?;
         ahs.push(ah);
      }
   }
   /// Done
